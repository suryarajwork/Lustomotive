...
var ShaderMount = class {
  ...
  textures: any = /* @__PURE__ */ new Map();
  minPixelRatio: any;
  maxPixelCount: any;
  isSafari = isSafari();
  uniformCache: any = {};
  textureUnitMap: any = /* @__PURE__ */ new Map();
  ...
  areUniformValuesEqual = (a: any, b: any): any => {
    if (a === b) return true;
...
function getShaderColorFromString(colorString: any): any {
...
var TICKET_LAYOUT = {
  ...
  stubTracking: 0,
  stubOpacity: 0.88,
  watermarkSize: 60 / REF,
  watermarkOpacity: 0.15,
  watermarkColor: "#ff1744",
  inkColor: "#ffffff"
};
var TICKET_TEXTURE = {
  engine: "generative",
...
function ticketClipPath(width: any, height: any, geometry = TICKET_GEOMETRY) {
...
function fitScale(lines: any, opts: any) {
  ...
  const longest = Math.max(...lines.map((l: any) => l.length));
...
function TicketCard({
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 drop-shadow-md" style={{ color: layout.inkColor, textShadow: "0px 1px 4px rgba(0,0,0,0.8)" }}>
        <div
        ...
        <div
          ...
          style={{
            ...
            textShadow: "0 0 10px rgba(255,23,68,0.8), 0 0 20px rgba(255,23,68,0.4)",
            left: layout.padding * width,
        ...
        >
          {lines.map((line: any, i: any) => <div key={i}>{line}</div>)}
        </div>
        {(venue || dates) && (
          <div
        ...
        {children && (
          <div className="absolute" style={{ left: layout.padding * width, top: layout.nameTop * width + (layout.nameLead * width * scale * lines.length) + (12 * (width / REF)), width: perfX - (layout.padding * width * 2), zIndex: 40 }}>
            {children}
          </div>
        )}
      </div>
...
function TiltCard({
  ...
  const glareRef = React.useRef<any>(null);
  ...
  const onMove = React.useCallback(
    (e: any) => {
      ...
      if (glareRef.current) {
        glareRef.current.style.background = `radial-gradient(38% 55% at ${(dx + 0.5) * 100}% ${(dy + 0.5) * 100}%, rgba(255,23,68,${glare}) 0%, rgba(255,23,68,0) 70%)`;
      }
    },
    [maxTilt, scale, glare]
  );
...
function playShutterSound({ volume = 0.35, gap = 0.045 } = {}) {
  const Ctor = typeof window !== "undefined" ? (window as any).AudioContext ?? (window as any).webkitAudioContext : void 0;
  if (!Ctor) return;
  audioCtx ??= new Ctor();
